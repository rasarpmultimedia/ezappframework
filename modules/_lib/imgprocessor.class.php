...
class ImgProcessor{
  ...
  private function calThumbRatio(){
       ...
       $ratio = ($this->width/$this->height);
	   $dim = ($this->thumb_width/$this->thumb_height);
	   if($dim > $ratio){
	   	$this->thumb_width  = $this->thumb_height*$ratio;
	   }else{
	   	$this->resize_height = $this->thumb_width/$ratio;
	   }
	    return true; 
  }
  ...
  public function addCenteredWatermark($target_file,$watermarkimg,$newimg){
	  ...
	  imagejpeg($targetimg,$newimg,90);
	  imagedestroy($targetimg);
	  imagedestroy($watermark);
    }
   
  public function addWatermark($target_file,$watermarkimg,$newimg){
   	  $watermark= imagecreatefrompng($watermarkimg);
	  imagealphablending($watermark, false);
	  imagesavealpha($watermark, true);
	  $targetimg = imagecreatefromjpeg($tartget_file);
