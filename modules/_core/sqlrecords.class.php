...
class SQLRecords{
  ...
  **/
    ...
    public function initQuery($tablename){
       return new SQLRecords($tablename); 
    }
    ...
    /* Custom Sql Select Statments Starts here */
	  ...
	  * @method query, $sql->query("select name,email from users where id=:id");
	  ...
	  * */
    public function query($prep_query){
	 ...
	 * @method joinRecords,
	 * @param  $join_table eg the second table to join $join_on eg page.category_id = category.id
...
class RecordSet extends SQLRecords{
    ...
    public function initQuery($tablename){
       return new RecordSet($tablename); 
    }
