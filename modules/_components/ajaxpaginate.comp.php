	...
	public function buildAjaxPagination(){
	 $output ="<ul>";
	  if($this->totalPage() > 0 || $this->totalPage()>=1){
		if($this->hasPreviousPage()){
		  ...
		  $output .= "<span class=\"active\"><a onclick=\"app.ajax.paginate.loadAjaxPage({$prev})\" href=\"javascript:void(0);\"> &laquo; Prev</a></span>";
		}else{
	 ...
	 for($i=1; $i <= ($this->totalPage());$i++){
		 if($i == $this->current_page){
		   ...
		   $output .= "&nbsp;<span class=\"active\"><a onclick=\"app.ajax.paginate.loadAjaxPage({$i})\" href=\"javascript:void(0);\">{$i}</a></span>"; 
		 }
	 }
	 ...
	 if(!$this->totalPage()){
		if($this->hasNextPage()){
		  $next = $this->nextPage();
		  $output .= "<li>&nbsp;<span class=\"active\"><a onclick=\"app.ajax.paginate.loadAjaxPage({$next})\" href=\"javascript:void(0);\">&laquo;Next</a></span></li>";
		}else{
